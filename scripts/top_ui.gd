...
var points_to_win:int = 1000
var current_count:int = 120
var number_attemps:int = 15
...
func _ready():
	#change this to switch the mode from "Timer"
	#to number of moves
	end_game_mode = false
	reset_game()
...
func reset_game():
	...
	if end_game_mode:
		...
		print("END GAME BY MOVES NUMBER!")
	else:
		counter_label.text = str(current_count)
		print("END GAME BY TIMER!")
